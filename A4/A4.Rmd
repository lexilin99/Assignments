...
\textbf{\underline{Answer:}} \underline{\textbf{Tobit model}} is optimal for dealing with the issue of censored data. 
...
tobit_like <- function(par, x1, x2, x3, x4, x5, x6, y){
  ...
  sigma <- par[8]
  pdf <- dnorm(y, y_hat, sigma, log = TRUE)
  cdf <- pnorm((log(100000)-y_hat)/sigma, log = TRUE)
  cdf[cdf > 0.999999] = 0.999999
  cdf[cdf < 0.000001] = 0.000001
  like <- ifelse(y >= log(100000), pdf, cdf)
  return(-sum(like)) 
}
...
par <- c(1,1,1)
tobit_res <- optim(par, fn = tobit_like, method = "BFGS", 
